# Orbitblade v5 — Historie

Dieses Dokument ist das **Archiv**. Es enthält die vollständige, chronologisch
gewachsene Fassung von `CLAUDE.md`, wie sie bis zum 30.08.2026 gepflegt wurde.

Verbindlich ist **ausschließlich `CLAUDE.md`**. Diese Datei wird nur gelesen, wenn
jemand wissen will, *warum* eine Entscheidung so gefallen ist — die Begründungen und
Messreihen darin sind weiterhin wertvoll. Sie wird nicht mehr fortgeschrieben.

Achtung: Teile dieses Archivs sind überholt. Insbesondere die Abschnitte zu
Orbitpfad v2 (14 Knoten, Rangleitern), die Lauflängen 19–23 min und 11:21 sowie die
Verweise auf `tools/vollerlauf.js` und `tools/pacing.js` gelten nicht mehr. Die
geprüften Werte stehen in `CLAUDE.md`.
