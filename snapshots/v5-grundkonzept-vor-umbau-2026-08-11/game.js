...
/* CHARAKTERE — aus den bisher rein kosmetischen Figuren werden echte Charaktere.
   Jeder hat einen Werteschnitt UND eine Besonderheit, die das Spielgefühl verändert,
   nicht nur die Zahlen. Bewusst zwei sehr verschiedene Pole: einer verzeiht Fehler,
   einer belohnt Genauigkeit. Die Startmächte bleiben davon unberührt — die wählst du
   weiterhin selbst, sonst würden sich zwei Systeme in die Quere kommen. */
const FIGUREN = {
  held:      { name:'Klingenläufer', desc:'Gepanzert, mit wehendem Umhang', start:true,
               hp:1.15, tempo:1.0, reichweite:1.0, fokusZiel:1.0, barriereMal:2.0,
               staerke:'Mehr Leben · Barriere baut sich doppelt so schnell auf',
               fuer:'Verzeiht Fehler — der Einstieg' },
  konstrukt: { name:'Konstrukt',     desc:'Schwebende Einheit mit abgesetzten Pods',
               hp:0.80, tempo:1.18, reichweite:0.92, fokusZiel:0.66, barriereMal:1.0,
               staerke:'Schneller · Fokus lädt nach 8 statt 12 Treffern',
               fuer:'Belohnt genaues Positionieren' },
};
...
const SAVE_KEY='orbitblade_konzept_save', SAVE_VERSION=5;
...
function sweetArcHalf(){
  return CONFIG.spinArcHalf * Math.pow(0.75, Math.max(0, (bonuses.blades||1)-1));
}
...
function newPlayer(){
  ...
  const hp = Math.round((CONFIG.playerHp + Math.round(metaValue('metaHp'))) * figur().hp);
...
function resetGame(){
  ...
  activeCd={wirbel:0,stoss:0,bombe:0,nova:0,sog:0}; phaserCd=0; dmgBoostUntil=0; shieldUntil=0; stossWaveT=0; wirbelT=0; spinHitTimer=0;
  ...
  wiederaufBenutzt=false; fokus=0; fokusBereit=false; endlosLauf=false; setzeLaufziele();
  toasts=[]; banner=null;
...
function randomEnemyType(){
  ...
  const panzerAb = hilfe().panzerAb || CONFIG.panzerAbWelle;
  if(wave>=panzerAb && r<0.22) return 'panzer';
...
function updateHUD(){
  ...
  // Fokus-Leiste
  const fw=document.getElementById('fokus-wrap'), fb=document.getElementById('fokus-bar');
  if(fw && fb){
    fb.style.width=Math.min(100, fokus/fokusZiel()*100)+'%';
    fw.classList.toggle('bereit', fokusBereit);
  }
  // Laufziele — nur neu bauen, wenn sich etwas geändert hat, sonst flackert es
  const zl=document.getElementById('ziel-liste');
  if(zl){
    const stand=laufziele.map(z=>z.def.id+':'+z.wert+':'+(z.fertig?1:0)).join('|');
    if(zl.dataset.stand!==stand){
      zl.dataset.stand=stand;
      zl.innerHTML='';
      for(const z of laufziele){
...
function update(dt){
  ...
  const pspeed=CONFIG.playerBaseSpeed*(1+bonuses.speed)*figur().tempo;
  ...
  if(spinHitTimer<=0){
    ...
    for(const en of enemies){
      ...
      const durchschlag = runAbilities.schneide>=SPRUNG_STUFE;
